...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, digests=None, learnt_p=False):
        B, N, C = x.shape
        # origin attention
        qkv = (
            self.qkv(x)
        ...
        if mask is not None:
            ...
            if digests is not None:
                mask = mask[:, start_pos:mask.size(1)]
            else:
                mask = mask[:, start_pos:]
...
class Block(nn.Module):
    def __init__(
        ...
        self.ab_norm1 = norm_layer(dim)
        self.ab_norm2 = norm_layer(dim)
        self.ab_attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
                                 qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.ab_mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.ab_drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x, mask=None, prompts=None, digests=None, learnt_p=False):
        ...
        if digests is not None:
            ab = digests
            ab, ab_attn = self.ab_attn(self.ab_norm1(ab))
            ab = ab + self.ab_drop_path(ab)
            ab = ab + self.ab_drop_path(self.ab_mlp(self.ab_norm2(ab)))
            x = torch.cat((x, ab), dim=1)
        _x, attn = self.attn(self.norm1(x), mask=mask, prompts=prompts, digests=digests,
                             learnt_p=learnt_p)
        x = x + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        if digests is not None:
            x, ab = x.split([x.shape[1]-1, 1], dim=1)
        return x, attn
